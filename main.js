...
const weddingDate = new Date("2025-07-26T08:00:00");
...
document.querySelectorAll('#side-select .select-toggle').forEach(toggle => {
    toggle.addEventListener('click', function () {
        document.querySelectorAll('#side-select .select-toggle').forEach(el => el.classList.remove('active'));
...
document.querySelectorAll('#status-select .select-toggle').forEach(toggle => {
    toggle.addEventListener('click', function () {
        document.querySelectorAll('#status-select .select-toggle').forEach(el => el.classList.remove('active'));
        this.classList.add('active');
...
document.getElementById('guest-form').addEventListener('submit', function (e) {
    ...
    db.ref('guests').push(data)
        .then(() => {
            ...
            document.querySelectorAll('.select-toggle').forEach(el => el.classList.remove('active'));
            document.querySelector('#side-select .select-toggle[data-value="Nhà trai"]').classList.add('active');
            document.querySelector('#status-select .select-toggle[data-value="Sẵn sàng"]').classList.add('active');
        })
        .catch(error => {
